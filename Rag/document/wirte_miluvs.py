...
def file_parser_process(dir_path: str, output_queue: Queue, batch_size: int = 20):
    """进程1：解析目录下所有md文件并分批放入队列,如果内存不够请减少batch_size"""
    ...
    while True:
        try:
            ...
            if isinstance(datas, list):
                ...
                print(f"累计已写入: {total_count} 个文档块")
        except Exception as e:
            print(f"写入数据失败 ！")
            print(e)

    print(f"写入进程结束，总计写入 {total_count} 个文档块")


if __name__ == '__main__':
    # 配置参数
    md_dir = r'C:\Users\33122\Desktop\ge_assistant\Rag\data\md'  # Markdown文件目录
